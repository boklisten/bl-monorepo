...
export class PaymentDibsHandler {
  ...
  public async handleDibsPayment(payment: Payment): Promise<Payment> {
    ...
    const dibsEasyOrder: DibsEasyOrder = await this.getDibsEasyOrder(userDetail, order);
  ...
  private getDibsEasyOrder(userDetail: UserDetail, order: Order): Promise<DibsEasyOrder> {
    if (order.delivery) {
