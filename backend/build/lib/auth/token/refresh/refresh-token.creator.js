...
function createPayload(username, userid) {
    return {
...
function create(username, userid) {
    return new Promise((resolve, reject) => {
        ...
        if (!username || !validator.isEmail(username))
            return reject(blError.msg("username is undefined or not an email").code(103));
        if (!userid || userid.length <= 0)
            return reject(blError.msg("userid is empty or undefined").code(103));
        jwt.sign(createPayload(username, userid), BlEnv.REFRESH_TOKEN_SECRET, { expiresIn: APP_CONFIG.token.refresh.expiresIn }, (error, refreshToken) => {
