...
export class TwilioSmsEventOperation {
    ...
    async parseAndAddTwilioEvent(twilioEvent, blMessageId) {
        if (!blMessageId) {
    ...
    async updateMessageWithTwilioSmsEvent(message, smsEvent) {
        const newSmsEvents = message.smsEvents && message.smsEvents.length > 0
            ? message.smsEvents
            : [];
