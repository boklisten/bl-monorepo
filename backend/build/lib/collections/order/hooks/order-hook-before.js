...
export class OrderHookBefore {
    ...
    validate(requestJsonBody) {
        return new Promise((resolve, reject) => {
            if (Object.prototype.toString.call(requestJsonBody) === "[object Array]") {
                return reject(new BlError("request is an array but should be a object")
            ...
            try {
                ...
                reject(new BlError("unkown error, request body is not valid")
                    .store("error", error)
                    .code(701));
            }
            resolve(true);
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    checkMinimumRequiredFields(requestBody) {
