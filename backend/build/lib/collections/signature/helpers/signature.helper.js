...
export async function deserializeBase64EncodedImage(base64EncodedImage) {
...
export async function userHasValidSignature(userDetail) {
    return (await getValidUserSignature(userDetail)) != null;
}
export function signatureIsValidForUser(userDetail, signature) {
    if (isSignatureExpired(signature)) {
        return false;
    }
    return isUnderage(userDetail) === signature.signedByGuardian;
}
export function isUnderage(userDetail) {
    const now = new Date();
...
export function isSignatureExpired(signature) {
    const now = new Date();
    const oldestAllowedSignatureTime = new Date(now.getFullYear(), now.getMonth() - SIGNATURE_NUM_MONTHS_VALID, now.getDate());
...
export async function signOrders(userDetail) {
    ...
    await Promise.all(orders
        ...
        }).catch((error) => logger.error(`While processing new signature, unable to update order ${order.id}: ${error}`));
    }));
}
export async function isGuardianSignatureRequired(userDetail) {
    if (!isUnderage(userDetail)) {
        return false;
    }
    const latestValidSignature = await getValidUserSignature(userDetail);
    return (latestValidSignature === null || !latestValidSignature.signedByGuardian);
}
