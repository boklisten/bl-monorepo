...
function generateBlIds(numberOfIds) {
    return Array.from({ length: numberOfIds }, () => Array.from({ length: BL_ID_LENGTH }, () => VALID_BL_ID_CHARACTERS[Math.floor(Math.random() * VALID_BL_ID_CHARACTERS.length)]).join(""));
}
function createBarcodeCanvas(id) {
...
function createQRCodeCanvas(id) {
...
function createBlIdCanvas(id) {
    ...
    printContext.drawImage(barcodeCanvas, qrcodeCanvas.width + PRINTER_DIMENSIONS.label.spaceBetween, 0);
    return blIdCanvas;
}
async function addIdPagesToDocument(id, document_) {
...
async function generateBlIdPDF() {
