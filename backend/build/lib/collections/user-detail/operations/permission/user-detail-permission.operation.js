...
export class UserDetailPermissionOperation {
    async run(blApiRequest) {
        const { data: parsedRequest, success, error, } = UserDetailPermissionSpec.safeParse(blApiRequest);
        if (!success) {
        ...
        const userDetail = await BlStorage.UserDetails.get(parsedRequest.documentId);
        ...
        const user = z
            ...
            .parse(users[0]);
        if (!PermissionService.isAdmin(parsedRequest.user.permission) ||
            !PermissionService.isPermissionOver(parsedRequest.user.permission, user.permission) ||
            !PermissionService.isPermissionOver(parsedRequest.user.permission, permissionChange)) {
