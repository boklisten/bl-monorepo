import { calculateItemImbalances, calculateUnmatchableItems, canMatchPerfectlyWithStand, copyUsers, countItemOccurrences, countMatchesForEachUser, removeFullyMatchedUsers, sortUsers, tryFindPartialMatch, tryFindTwoWayMatch, updateItemImbalances, } from "@backend/lib/collections/user-match/helpers/match-finder/match-utils.js";
...
export class MatchFinder {
    ...
    unmatchableItems = new Set();
    userMatches = [];
    standMatches = [];
    MAX_USER_MATCH_COUNT = 2;
    constructor(_users) {
        this._users = _users;
    ...
    createTwoWayMatches() {
        ...
        while (this.userGroups.size > 1) {
            ...
            for (const user of this.users) {
                if (user.groupMembership === bestGrouping.groupA ||
            ...
            this.logMatchingStatus(`Created ${bestGrouping.numberOfMatches} TwoWayMatches for new group ${joinedGroupId} (${(bestGrouping.successRate * 100).toPrecision(3)}% match)`);
            this.logGroupStatus("Current");
        }
        this.logMatchingStatus("TwoMatching Complete");
    }
    groupUsersByMembership() {
    ...
    verifyMatches() {
        ...
        for (const userMatch of this.userMatches) {
            ...
            if (userMatch.expectedAToBItems.difference(originalCustomerA.items).size >
                0 ||
            ...
            if (userMatch.expectedBToAItems.difference(originalCustomerA.wantedItems)
                .size > 0 ||
            ...
            originalCustomerA.wantedItems = originalCustomerA.wantedItems.difference(userMatch.expectedBToAItems);
            originalCustomerB.items = originalCustomerB.items.difference(userMatch.expectedBToAItems);
            originalCustomerB.wantedItems = originalCustomerB.wantedItems.difference(userMatch.expectedAToBItems);
        }
        if (Object.values(userMatchCounts).some((userMatchCount) => userMatchCount > 2)) {
            throw new BlError("Users cannot have more than two user matches!");
        }
        const usersWithStandMatch = new Set();
        for (const standMatch of this.standMatches) {
            ...
            if (standMatch.expectedHandoffItems.difference(originalCustomer.items)
                .size > 0) {
                throw new BlError("Users cannot give away more books than they have!");
            }
            if (standMatch.expectedPickupItems.difference(originalCustomer.wantedItems)
                .size > 0) {
                throw new BlError("Users cannot receive books they do not want!");
            }
            originalCustomer.items = originalCustomer.items.difference(standMatch.expectedHandoffItems);
            originalCustomer.wantedItems = originalCustomer.wantedItems.difference(standMatch.expectedPickupItems);
        }
    ...
    standMatchUnmatchableItems() {
        ...
        if (uncheckedUnmatchableItems.size > 0) {
            ...
            logger.debug(`🕵️‍♀️ Detected unmatchable items: ${Array.from(uncheckedUnmatchableItems).join(", ")}`);
     ...
     * @private
     */
    createFullStandMatches() {
        const itemCounts = countItemOccurrences(this.users.flatMap((user) => Array.from(user.items)));
        const wantedItemCounts = countItemOccurrences(this.users.flatMap((user) => Array.from(user.wantedItems)));
        ...
        this.logMatchingStatus("Tried to create imbalance minimizing stand matches, here are the results");
        this.logItemDemand(this.users);
    }
    /**
     ...
     * @private
     */
    createImbalanceMinimizingStandMatches(users, itemImbalances) {
     ...
     * @private
     */
    createStandMatchIfReachedMatchLimit(user) {
        const userMatches = this.userMatches.filter((userMatch) => userMatch.customerA === user.id || userMatch.customerB === user.id);
     ...
     * @private
     */
    evaluateTwoWayMatchRatio(userGroup) {
        ...
        const notMatched = localUserPool.filter((user) => user.wantedItems.size > 0 || user.items.size > 0);
        return {
            numberOfMatches: matchCount,
            successRate: notMatched.length === 0
                ? 1
     ...
     * @private
     */
    createMatches(matchFinder, userGroup) {
    ...
    createStandMatch(user, items) {
        ...
        const existingStandMatch = this.standMatches.find((standMatch) => standMatch.customer === user.id);
    ...
    logMatchingStatus(message) {
        logger.debug(`🔗${message} - Currently at ${this.userMatches.length} UserMatches and ${this.standMatches.length} StandMatches. ${this.users.length} users remaining.`);
    }
    logGroupStatus(message) {
        if (this.userGroups.size === 0)
            return;
        logger.debug(` ${message} Groups (${this.userGroups.size}): ${this.userGroups
            .entries()
            .toArray()
            .sort((a, b) => a[0].localeCompare(b[0]))
    ...
    logPerformanceMetrics() {
        ...
        logger.debug(`Started with ${this._users.length} users with ${totalItems} books, wanting ${totalWants} books`);
        ...
        logger.debug(`${onlyUserMatches} users (${((100 * onlyUserMatches) / this._users.length).toPrecision(3)}%) will only have user matches, while ${this.standMatches.length} users (${((100 * this.standMatches.length) / this._users.length).toPrecision(3)}%) will need to visit the stand`);
        ...
        const matchConfigSetups = new Map();
        ...
        for (const standInAndOut of standInAndOuts) {
            logger.debug(`${standInAndOut}: ${pickupCounts.get(standInAndOut)} pickups and ${handoffCounts.get(standInAndOut)} deliveries`);
        }
    }
}
