 ...
 */
export function sortUsers(users, standMatches) {
 ...
 */
export function removeFullyMatchedUsers(users) {
 ...
 */
export function tryFindTwoWayMatch(user, userGroup) {
    return (userGroup.find((candidate) => user.id !== candidate.id &&
        !(user.wantedItems.symmetricDifference(candidate.items).size > 0) &&
        !(candidate.wantedItems.symmetricDifference(user.items).size > 0)) ?? null);
}
/**
 ...
 */
export function tryFindPartialMatch(user, userGroup) {
    ...
    for (const candidate of userGroup) {
        ...
        const userToBestCandidateItems = user.items.intersection(bestCandidate?.wantedItems ?? new Set());
        const bestCandidateToUserItems = (bestCandidate?.items ?? new Set()).intersection(user.wantedItems);
        const candidateMatchNumberOfItems = userToCandidateItems.size + candidateToUserItems.size;
        const bestCandidateMatchNumberOfItems = userToBestCandidateItems.size + bestCandidateToUserItems.size;
...
export function countItemOccurrences(items) {
    return items.reduce((accumulator, next) => ({
        ...accumulator,
 ...
 * @private
 */
export function calculateItemImbalances(itemsCounts, wantedItemsCounts) {
 ...
 *
 **/
export function canMatchPerfectlyWithStand(user, itemImbalances) {
    const canDeliverItems = Array.from(user.items).every((item) => (itemImbalances[item] ?? 0) > 0);
    const canReceiveWantedItems = Array.from(user.wantedItems).every((item) => (itemImbalances[item] ?? 0) < 0);
 ...
 * @private
 */
export function updateItemImbalances(items, wantedItems, itemImbalances) {
 ...
 */
export function calculateUnmatchableItems(users) {
...
export function countMatchesForEachUser(userMatches, standMatches) {
