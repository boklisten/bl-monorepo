import { MatchLocationSchema, } from "@backend/lib/collections/user-match/helpers/match-finder/match-types.js";
 ...
 */
export async function getMatchableUsers(branchIds, deadlineBefore, includeSenderItemsFromOtherBranches) {
    ...
    return await Promise.all(matchableUsers.map(async (user) => {
 ...
 * @param includeSenderItemsFromOtherBranches whether the remainder of the items that a customer has in possession should be added to the match, even though they were not handed out at the specified branchIds
 */
async function getMatchableSender(branchIds, deadlineBefore, includeSenderItemsFromOtherBranches) {
 ...
 * @param branchIds The IDs of branches to search for users and items
 */
async function getMatchableReceivers(branchIds) {
