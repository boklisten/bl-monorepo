...
export class MatchNotifyOperation {
    async run(blApiRequest) {
        ...
        return new BlapiResponse(await Promise.allSettled((await BlStorage.UserDetails.getMany([...targetCustomerIds])).map(async (customer) => {
            await sendSMS(customer.phone, `Hei, ${customer.name.split(" ")[0]}. ${parsedRequest.data.message} Mvh Boklisten`);
            await sendMail({
                userId: customer.id,
