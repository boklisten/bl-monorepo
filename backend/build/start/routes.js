...
router.get("/auth/facebook/callback", async ({ ally, request, response }) => {
    ...
    const userAndTokens = await UserProvider.loginOrCreate(user.email, provider, user.id);
    const redirectUrl = `${retrieveRefererPath(request.headers()) ?? BlEnv.CLIENT_URI}auth/token?access_token=${userAndTokens.tokens.accessToken}&refresh_token=${userAndTokens.tokens.refreshToken}`;
