...
test.group("AccessTokenCreator", (group) => {
    ...
    group.each.setup(async () => {
        ...
        testRefreshToken = await RefreshTokenCreator.create(testUsername, testUserid);
    });
    test("should reject with BlError when username is undefined", async () => {
        const username = undefined;
        AccessTokenCreator.create(
        // @ts-expect-error fixme: auto ignored
        username, testUserid, testPermission, testUserDetailId, testRefreshToken).should.be.rejectedWith(BlError);
    });
    test("should reject with BlError when username is empty", async () => {
        const username = "";
        AccessTokenCreator.create(username, testUserid, testPermission, testUserDetailId, testRefreshToken).should.be.rejectedWith(BlError);
    });
    test("should reject with BlError when userId is undefined", async () => {
        const userid = undefined;
        AccessTokenCreator.create(testUsername, 
        // @ts-expect-error fixme: auto ignored
        userid, testPermission, testUserDetailId, testRefreshToken).should.be.rejectedWith(BlError);
    });
    test("should should reject with BlError when requestToken is undefined", async () => {
        const refreshToken = "";
        AccessTokenCreator.create(testUsername, testUserid, testPermission, testUserDetailId, refreshToken).should.be.rejectedWith(BlError);
    });
    test("should reject with BlError code 905 when refreshToken is not valid", async () => {
        const refreshToken = "this is not valid";
        AccessTokenCreator.create(testUsername, testUserid, testPermission, testUserDetailId, refreshToken).then((accessToken) => {
    ...
    test("should resolve with a accessToken when all parameters is valid", async () => {
        AccessTokenCreator.create(testUsername, testUserid, testPermission, testUserDetailId, testRefreshToken).then((accessToken) => {
