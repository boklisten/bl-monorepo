...
test.group("ConfirmPendingPasswordResetOperation", (group) => {
    const confirmPendingPasswordResetOperation = new ConfirmPendingPasswordResetOperation();
    let testBlApiRequest;
    let testPendingPasswordReset;
    let localLoginUpdateSuccess;
    ...
    test("should reject if documentId is not found", async () => {
        ...
        return expect(confirmPendingPasswordResetOperation.run(testBlApiRequest)).to.be.rejectedWith(BlError, /PendingPasswordReset "notFoundPasswordReset" not found/);
    });
    test("should reject if token does not match", async () => {
        // @ts-expect-error fixme: auto ignored
        testBlApiRequest.data["resetToken"] = "notAValidToken";
        return expect(confirmPendingPasswordResetOperation.run(testBlApiRequest)).to.be.rejectedWith(BlError, /Invalid password reset attempt: computed token hash does not match stored hash/);
    });
    test("should reject if blApiRequest.data.newPassword is null, empty or undefined", async () => {
        ...
        return expect(confirmPendingPasswordResetOperation.run(testBlApiRequest)).to.be.rejectedWith(BlError, /blApiRequest.data.newPassword is null, empty or undefined/);
    });
    test("should reject if blApiRequest.data.newPassword is under length of 6", async () => {
        // @ts-expect-error fixme: auto ignored
        testBlApiRequest.data["newPassword"] = "abcde";
        return expect(confirmPendingPasswordResetOperation.run(testBlApiRequest)).to.be.rejectedWith(BlError, /blApiRequest.data.newPassword is under length of 6/);
    });
    test("should reject if PendingPasswordReset is expired", async () => {
        testPendingPasswordReset.creationTime = new Date();
        testPendingPasswordReset.creationTime.setDate(testPendingPasswordReset.creationTime.getDate() - 8);
        return expect(confirmPendingPasswordResetOperation.run(testBlApiRequest)).to.be.rejectedWith(BlError, /PendingPasswordReset (.*) expired/);
    });
    ...
    test("should reject if LocalLoginHandler.setPassword rejects", async () => {
        ...
        return expect(confirmPendingPasswordResetOperation.run(testBlApiRequest)).to.be.rejectedWith(BlError, /Could not update localLogin with password/);
    });
    test("should reject if PendingPasswordReset inactive", async () => {
        testPendingPasswordReset.active = false;
        return expect(confirmPendingPasswordResetOperation.run(testBlApiRequest)).to.be.rejectedWith(BlError, /PendingPasswordReset (.*) already used/);
    });
