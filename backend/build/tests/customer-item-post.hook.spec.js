...
test.group("CustomerItemPostHook", (group) => {
    let sandbox;
    let testCustomerItem;
    let testOrder;
    let testAccessToken;
    let validateCustomerItem;
    let testUserDetail;
    ...
    test("should reject if customerItem parameter is undefined", async () => {
        return expect(
        // @ts-expect-error fixme: auto ignored
        customerItemPostHook.before(undefined, testAccessToken)).to.be.rejectedWith(BlError, /customerItem is undefined/);
    });
    test("should reject if customerItemValidator.validate rejects", async () => {
        validateCustomerItem = false;
        return expect(customerItemPostHook.before(testCustomerItem)).to.be.rejectedWith(BlError, "could not validate customerItem");
    });
    ...
    test("should reject if userDetail is not valid", async () => {
        ...
        return expect(customerItemPostHook.before(testCustomerItem)).to.be.rejectedWith(BlError, /userDetail "userDetail1" not valid/);
    });
    test("should reject if customerItems are empty", async () => {
        return expect(customerItemPostHook.after([], testAccessToken)).to.be.rejectedWith(BlError, /customerItems is empty or undefined/);
    });
    test("should reject if customerItem.customer is not defined", async () => {
        testCustomerItem.customer = "notFoundCustomer";
        return expect(customerItemPostHook.after([testCustomerItem], testAccessToken)).to.be.rejectedWith(BlError, /userDetail not found/);
    });
    test("should update userDetail with the ids array if it was empty", async () => {
        ...
        customerItemPostHook.after([testCustomerItem], testAccessToken).then(() => {
            return expect(userDetailStub.calledWithMatch("userDetail1", {
    ...
    test("should reject with error if customerItems.orders.length is over 1", async () => {
        ...
        return expect(customerItemPostHook.after([testCustomerItem], testAccessToken)).to.be.rejectedWith(BlError, /customerItem.orders.length is "2" but should be "1"/);
    });
