...
test.group("DbQueryBooleanFilter", async () => {
    const dbQueryBooleanFilter = new DbQueryBooleanFilter();
    ...
    test('should return array of [{fieldName: "haveEaten", value: true"}]', async () => {
        ...
        expect(dbQueryBooleanFilter.getBooleanFilters({ haveEaten: "true" }, [
            "haveEaten",
        ])).to.eql(result);
    });
    ...
    test("should return array that includes all params that are of boolean type in query", async () => {
        ...
        expect(dbQueryBooleanFilter.getBooleanFilters({ confirmed: "true", hasCar: "false", isOld: "true" }, ["confirmed", "hasCar", "isOld", "haveChildren"])).to.eql(result);
    });
});
