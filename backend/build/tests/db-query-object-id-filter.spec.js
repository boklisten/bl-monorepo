...
test.group("DbQueryObjectIdFilter", async () => {
    const dbQueryObjectIdFilter = new DbQueryObjectIdFilter();
    test("should return empty array if query is valid and validObjectIdParams is empty", async () => {
        expect(dbQueryObjectIdFilter.getObjectIdFilters({ name: "5c2e0e5bb311ba0701f15967" }, [])).to.eql([]);
    });
    ...
    test("should not change values in query that are not in ValidObjectIdParams", async () => {
        ...
        expect(dbQueryObjectIdFilter.getObjectIdFilters({ id: "5c2e0e5bb311ba0701f15967" }, ["id"])).to.eql(result);
    });
    test("should return correct array given valid input", async () => {
        ...
        expect(dbQueryObjectIdFilter.getObjectIdFilters(query, [
            "id",
            "customer",
            "branch",
        ])).to.eql(result);
