...
test.group("DibsPaymentService", (group) => {
    ...
    let testDibsEasyPaymentResponse;
    let httpHandlerGetSuccess;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const testUser = {};
    ...
    test("should reject if httpHandler rejects", async () => {
        ...
        expect(dibsPaymentService.fetchDibsPaymentData("dibsPaymentId1")).to.be.rejectedWith(BlError, /could not get payment details for paymentId "dibsPaymentId1"/);
    });
    test("should reject if dibsResponse does not include a payment", async () => {
        ...
        dibsPaymentService
            ...
            .catch((err) => {
            // @ts-expect-error fixme: auto ignored
            expect(err.errorStack[0].getMsg()).to.be.eq("dibs response did not include payment information");
        });
    });
    test("should resolve with a dibsEasyPayment object with correct paymentId", async () => {
        ...
        expect(dibsPaymentService.fetchDibsPaymentData("aPaymentId")).to.eventually.be.eql({ paymentId: "aPaymentId" });
    });
});
