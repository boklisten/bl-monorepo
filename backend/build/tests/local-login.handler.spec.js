...
test.group("LocalLoginHandler", (group) => {
    ...
    test("should reject with blError.code 702 when username is not found in db", async () => {
        LocalLoginHandler.get("notFound@mail.com").catch((blError) => {
    ...
    test("should reject if localLogin is not found", async () => {
        ...
        return expect(LocalLoginHandler.setPassword(testUsername, "password")).to.be.rejectedWith(BlError, /localLogin was not found with username "notFound@mail.com/);
    });
    test("should reject if password is less than 6 characters", async () => {
        const testPassword = "short";
        return expect(LocalLoginHandler.setPassword(testUsername, testPassword)).to.be.rejectedWith(BlError, /localLogin password to short/);
    });
    test("should reject if localLogin is not found by username", async () => {
        const notFoundUsername = "notFound@mail.com";
        return expect(LocalLoginHandler.setPassword(notFoundUsername, "password")).to.be.rejectedWith(BlError, /localLogin was not found with username "notFound@mail.com"/);
    });
