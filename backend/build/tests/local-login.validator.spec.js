...
test.group("LocalLoginValidator", (group) => {
    ...
    group.each.setup(() => {
        ...
        sandbox.stub(LocalLoginHandler, "get").callsFake((username) => {
            return new Promise((resolve, reject) => {
                if (username === testLocalLogin.username)
        ...
        sandbox
            ...
            .callsFake((username, provider, providerId) => {
    ...
    test("username is not an email", async () => {
        testUserName = "bill";
        LocalLoginValidator.validate(testUserName, testPassword).should.be.rejectedWith(BlError);
    });
    ...
    test("username does not exist", async () => {
        ...
        LocalLoginValidator.validate(testUserName, testPassword).then((value) => {
    ...
    test("should resolve with correct provider and providerId when username and password is correct", async () => {
        ...
        return new Promise((resolve, reject) => {
            LocalLoginValidator.validate(testUserName, testPassword).then((returnedProvider) => {
    ...
    test("should reject with BlError if username does exist", async () => {
        ...
        LocalLoginValidator.create(username, password).then((value) => {
            value.should.not.be.fulfilled;
        }, (error) => {
    ...
    test("should resolve with provider and providerId if username and password is valid", async () => {
        ...
        const providerAndProviderId = await LocalLoginValidator.create(username, password);
