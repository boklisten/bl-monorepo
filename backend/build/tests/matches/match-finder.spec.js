...
import { createFakeMatchableUser, createFakeStandMatch, createFakeUserMatch, createUserGroup, seededRandom, shuffler, createMatchableUsersWithIdSuffix, } from "@backend/lib/collections/user-match/helpers/match-finder/match-testing-utils.js";
...
test.group("Full User Match", async () => {
    ...
    test("should full match after removing excessive delivery items", async () => {
        ...
        const [userMatches, standMatches] = matchFinder.generateMatches();
        const expectedUserMatch = createFakeUserMatch(mathias, beate, new Set(["book1", "book2", "book3"]));
...
test.group("Partly User Match", async () => {
    test("should create one full match and one partly", async () => {
        ...
        const mathiasXmons = createFakeUserMatch(mons, mathias, [], mathias.items.intersection(mons.wantedItems));
        const mathiasXstand = createFakeStandMatch(mathias, new Set(), mathias.items.difference(monika.items));
    ...
    test("should be able to fully match and partly match a set of shuffled users", async () => {
        ...
        // B => B
        // C => B, C
        const matchFinder = new MatchFinder(shuffle([
            senderGroupA,
            senderGroupB,
            senderGroupC,
            receiverGroupA,
    ...
    test("should be able to fully and partly match some ordered users, leaving some receivers to match with stand", async () => {
        ...
        const matchFinder = new MatchFinder([senderGroupA, senderGroupB, senderGroupC, receiverGroupA].flat());
        const [userMatches, standMatches] = matchFinder.generateMatches();
    ...
    test("should be able to fully and partly match some shuffled users, leaving some receivers to match with stand", async () => {
        ...
        const senderGroupC = createUserGroup("sender-C", 5, ["A", "B"], []);
        const receiverGroupA = createUserGroup("receiver-A", 10, [], ["A", "B"]);
        const matchFinder = new MatchFinder(shuffle([senderGroupA, senderGroupB, senderGroupC, receiverGroupA].flat()));
        const [userMatches, standMatches] = matchFinder.generateMatches();
        // 5 matches made in heaven
        // 3 senderA => receiverA
        // 4 senderB => receiverA
...
test.group("Large User Groups", async () => {
    ...
    test("can perfectly match realistic user data with itself", async () => {
        ...
        const matchFinder = new MatchFinder(shuffle([test_senders.slice(), test_receivers.slice()].flat()));
        const [userMatches, standMatches] = matchFinder.generateMatches();
    ...
    test("can sufficiently match realistic user data with a modified version of itself", async () => {
        ...
        const test_senders = createMatchableUsersWithIdSuffix(rawData, true);
        const test_receivers = createMatchableUsersWithIdSuffix(rawData, false);
        const matchFinder = new MatchFinder(shuffle([test_senders.slice(33), test_receivers.slice(20)].flat()));
        const [userMatches, standMatches] = matchFinder.generateMatches();
    ...
    test("should have a lot of pickup and deliveries when many new books are introduced", async () => {
        ...
        const testUsersYear1 = otto_treider_test_users_year_1.map(({ items, id }) => ({
            wantedItems: new Set(items),
            items: new Set(),
            groupMembership: "unknown",
            id: id + "_year1",
        }));
        const testUsersYear2 = otto_treider_test_users_year_2.map(({ items, id }) => ({
            items: new Set(items),
            wantedItems: new Set(),
            groupMembership: "unknown",
            id: id + "_year2",
    ...
    test("should be able to sufficiently match two different year classes with similar books", async () => {
        ...
        const testUsersYear0 = otto_treider_test_users_year_0.map(({ items, id }) => ({
            wantedItems: new Set(items),
            items: new Set(),
            groupMembership: "unknown",
            id: id + "_year0",
        ...
        const [userMatches, standMatches] = new MatchFinder(shuffle([testUsersYear0, testUsersYear1].flat())).generateMatches();
        const standDeliveryItems = standMatches.flatMap((standMatch) => Array.from(standMatch.expectedHandoffItems));
        const standPickupItems = standMatches.flatMap((standMatch) => Array.from(standMatch.expectedPickupItems));
        expect(standDeliveryItems.every((deliveryItem) => !standPickupItems.includes(deliveryItem)));
...
test.group("Group matching logic", async () => {
    test("should prioritize matching similar groups first", async () => {
        ...
        const [userMatches, standMatches] = matchFinder.generateMatches();
        expect(userMatches.every((userMatch) => !userMatch.customerA.includes("group3") &&
            !userMatch.customerB.includes("group3"))).to.equal(true);
...
test.group("Faulty data checks", async () => {
    test("should throw an error if the same user is listed twice in MatchableUsers", async () => {
        ...
        expect(() => new MatchFinder([userDuplicateA, userDuplicateB])).to.throw(BlError);
    });
