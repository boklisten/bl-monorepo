...
test.group("OrderItemBuyValidator", (group) => {
    ...
    test("should resolve when a valid order is passed", async () => {
        return expect(orderItemPriceValidator.validate(testBranch, 
        // @ts-expect-error fixme: auto ignored
        testOrder.orderItems[0], testItem)).to.eventually.be.fulfilled;
    });
    test("should reject if the orderItem.taxRate is not the same as item.taxRate", async () => {
        ...
        return expect(orderItemPriceValidator.validate(testBranch, 
        // @ts-expect-error fixme: auto ignored
        testOrder.orderItems[0], testItem)).to.be.rejectedWith(BlError, /orderItem.taxRate "0.75" is not equal to item.taxRate "0.33"/);
    });
    test("should reject if the orderItem.taxAmount is not equal to (item.price * item.taxRate)", async () => {
        ...
        return expect(orderItemPriceValidator.validate(testBranch, 
        // @ts-expect-error fixme: auto ignored
        testOrder.orderItems[0], testItem)).to.be.rejectedWith(BlError, /orderItem.taxAmount "100" is not equal to \(orderItem.amount "300" \* item.taxRate "0.5"\) "150"/);
    });
    test("should reject when item.price is 200 and orderItem.amount is 100", async () => {
        ...
        return expect(orderItemPriceValidator.validate(testBranch, 
        // @ts-expect-error fixme: auto ignored
        testOrder.orderItems[0], testItem)).to.be.rejectedWith(BlError, /orderItem.amount "100" is not equal to item.price "200" - orderItem.discount "0" = "200"/);
    });
    test("should reject if item.price is 134 and orderItem.amount is 400", async () => {
        ...
        return expect(orderItemPriceValidator.validate(testBranch, 
        // @ts-expect-error fixme: auto ignored
        testOrder.orderItems[0], testItem)).to.be.rejectedWith(BlError, /orderItem.amount "400" is not equal to item.price "134" - orderItem.discount "0" = "134"/);
    });
    test("should resolve if a valid order is sent", async () => {
        ...
        return expect(orderItemPriceValidator.validate(testBranch, 
        // @ts-expect-error fixme: auto ignored
        testOrder.orderItems[0], testItem)).to.be.fulfilled;
    });
    test("should reject if orderItem.taxAmount is not equal to ((item.price - discount.amount) * item.taxRate)", async () => {
        ...
        return expect(orderItemPriceValidator.validate(testBranch, 
        // @ts-expect-error fixme: auto ignored
        testOrder.orderItems[0], testItem)).to.be.rejectedWith(BlError, /orderItem.taxAmount "100" is not equal to \(orderItem.amount "400" \* item.taxRate "0.5"\) "200"/);
    });
    test("should reject if (item.price - discount.amount) is 400 but orderItem.amount is 100", async () => {
        ...
        return expect(orderItemPriceValidator.validate(testBranch, 
        // @ts-expect-error fixme: auto ignored
        testOrder.orderItems[0], testItem)).to.be.rejectedWith(BlError, /orderItem.amount "100" is not equal to item.price "500" - orderItem.discount "100" = "400"/);
    });
    test("should reject if (item.price - discount.amount) is 200 but orderItem.amount is 560", async () => {
        ...
        return expect(orderItemPriceValidator.validate(testBranch, 
        // @ts-expect-error fixme: auto ignored
        testOrder.orderItems[0], testItem)).to.be.rejectedWith(BlError, /orderItem.amount "560" is not equal to item.price "500" - orderItem.discount "300" = "200"/);
    });
