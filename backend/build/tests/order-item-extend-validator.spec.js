...
test.group("OrderItemExtendValidator", (group) => {
    ...
    test('should reject if orderItem.type is not "extend"', async () => {
        ...
        return expect(
        // @ts-expect-error fixme: auto ignored
        orderItemExtendValidator.validate(testBranch, testOrder.orderItems[0])).to.be.rejectedWith(BlError, /orderItem.type "rent" is not "extend"/);
    });
    test("should reject if orderItem.info.periodType is not allowed at branch", async () => {
        ...
        return expect(
        // @ts-expect-error fixme: auto ignored
        orderItemExtendValidator.validate(testBranch, testOrder.orderItems[0])).to.be.rejectedWith(BlError, /orderItem.info.periodType is "year" but it is not allowed by branch/);
    });
    ...
    test("should reject if orderItem.info is not defined", async () => {
        ...
        return expect(
        // @ts-expect-error fixme: auto ignored
        orderItemExtendValidator.validate(testBranch, testOrder.orderItems[0])).to.be.rejectedWith(BlError, /orderItem.info is not defined/);
    });
    test("should reject if orderItem.customerItem is not defined", async () => {
        ...
        return expect(
        // @ts-expect-error fixme: auto ignored
        orderItemExtendValidator.validate(testBranch, testOrder.orderItems[0])).to.be.rejectedWith(BlError, /orderItem.info.customerItem is not defined/);
    });
    test("should reject when customerItem have been extended to many times", async () => {
        ...
        return expect(
        // @ts-expect-error fixme: auto ignored
        orderItemExtendValidator.validate(testBranch, testOrder.orderItems[0])).to.be.rejectedWith(BlError, /orderItem can not be extended any more times/);
    });
});
