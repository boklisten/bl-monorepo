...
test.group("OrderItemRentPeriodValidator", (group) => {
    ...
    let orderStorageGetStub;
    let sandbox;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let branchPaymentInfo;
    const orderB = {
        id: "orderB",
        amount: 200,
        orderItems: [
    ...
    test("should reject if period is not found in branchPaymentInfo", async () => {
        ...
        return expect(orderItemRentPeriodValidator.validate(orderItem, branchPaymentInfo, 100)).to.be.rejectedWith(BlError, /rent period "semester" is not valid on branch/);
    });
    test("should reject if not all amounts is equal to 0 on orderItem", async () => {
        ...
        return expect(orderItemRentPeriodValidator.validate(orderItem, branchPaymentInfo, 100)).to.be.rejectedWith(BlError, /amounts where set on orderItem when branch is responsible/);
    });
    test("should resolve with true if all amounts is 0", async () => {
        ...
        return expect(orderItemRentPeriodValidator.validate(orderItem, branchPaymentInfo, 100)).to.be.fulfilled;
    });
    branchPaymentInfo = {
        responsible: false,
        rentPeriods: [
    ...
    test("should reject if period is the same but orderItem.amount is not 0", async ({ assert, }) => {
        ...
        await assert.rejects(() => orderItemRentPeriodValidator.validate(orderItem, branchPaymentInfo, 100));
    });
    test("should reject if orderItem.amount is not equal to rentPrice - oldOrderItem.amount ", async ({ assert, }) => {
        branchPaymentInfo = {
            responsible: false,
            rentPeriods: [
                {
        ...
        await assert.rejects(() => orderItemRentPeriodValidator.validate(orderItem, branchPaymentInfo, itemPrice));
    });
    test("should reject if orderItem.amount is not equal to rentPrice 500 - oldOrderItem.amount 750 = -250", async ({ assert, }) => {
        ...
        orderItem.info.periodType = "semester";
        // fixme, test is wrong
        await assert.doesNotReject(() => orderItemRentPeriodValidator.validate(orderItem, branchPaymentInfo, itemPrice));
    });
    test("should resolve if orderItem.amount is equal to rentPrice 500 - oldOrderItem.amount 750 = -250", async () => {
        ...
        orderItem.type = "rent";
        orderItem.info.periodType = "semester";
        return expect(orderItemRentPeriodValidator.validate(orderItem, branchPaymentInfo, itemPrice)).to.be.rejected;
    });
    test("should reject if orderItem.amount is not equalt to branchPayment percentage * itemPrice", async () => {
        ...
        return expect(orderItemRentPeriodValidator.validate(orderItem, branchPaymentInfo, itemPrice)).to.be.rejectedWith(BlError, /orderItem.amount "0" is not equal to itemPrice "100" \* percentage "0.5" "50"/);
    });
    test("should resolve if given valid orderItem", async () => {
        ...
        return expect(orderItemRentPeriodValidator.validate(orderItem, branchPaymentInfo, itemPrice)).to.be.fulfilled;
    });
});
