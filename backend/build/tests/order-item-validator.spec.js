...
test.group("OrderItemValidator", (group) => {
    ...
    const orderItemValidator = new OrderItemValidator(orderItemFieldValidator, orderItemRentValidator, orderItemBuyValidator, orderItemExtendValidator);
    let testOrder;
    let testBranch;
    ...
    test("should reject with error when order.amount is 500 and total of orderItems is 250", async () => {
        ...
        return expect(orderItemValidator.validate(testBranch, testOrder, false)).to.be.rejectedWith(BlError, /order.amount is "500" but total of orderItems amount is "250"/);
    });
    test("should reject with error when order.amount is 100 and total of orderItems is 780", async () => {
        ...
        return expect(orderItemValidator.validate(testBranch, testOrder, false)).to.be.rejectedWith(BlError, /order.amount is "100" but total of orderItems amount is "780"/);
    });
    test("should reject if amount does not include taxAmount", async () => {
        ...
        testOrder.amount = 100;
        return expect(orderItemValidator.validate(testBranch, testOrder, false)).to.be.rejectedWith(BlError, /orderItem.amount "100" is not equal to orderItem.unitPrice "100" \+ orderItem.taxAmount "25"/);
    });
    test("should reject if amount does not includes more than unitPrice + taxAmount", async () => {
        ...
        return expect(orderItemValidator.validate(testBranch, testOrder, false)).to.be.rejectedWith(BlError, /orderItem.amount "160" is not equal to orderItem.unitPrice "100" \+ orderItem.taxAmount "25"/);
    });
    test("should reject if taxAmount does not equal unitPrice * taxRate", async () => {
        ...
        testOrder.amount = 100;
        return expect(orderItemValidator.validate(testBranch, testOrder, false)).to.be.rejectedWith(BlError, /orderItem.taxAmount "0" is not equal to orderItem.unitPrice "100" \* orderItem.taxRate "0.25"/);
    });
    ...
    test("should reject if deadline is in the past and user is not admin", async () => {
        ...
        testOrder.amount = 125;
        return expect(orderItemValidator.validate(testBranch, testOrder, false)).to.eventually.be.rejectedWith(BlError, /orderItem deadlines must be in the future/);
    });
    test("should reject if deadline is more than four years into the future and user is not admin", async () => {
        ...
        testOrder.amount = 125;
        return expect(orderItemValidator.validate(testBranch, testOrder, false)).to.eventually.be.rejectedWith(BlError, /orderItem deadlines must less than two years into the future/);
    });
