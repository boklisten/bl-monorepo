...
test.group("OrderPlacedHandler", (group) => {
    let testOrder;
    let testPayment;
    let paymentsConfirmed;
    let testAccessToken;
    let orderUpdate;
    let testUserDetail;
    let userDeatilUpdate;
    ...
    const orderPlacedHandler = new OrderPlacedHandler(paymentHandler, customerItemHandler, orderItemMovedFromOrderHandler);
    let sandbox;
    group.each.setup(() => {
    ...
    test("should reject if userDetailStorage.updates rejects", async () => {
        ...
        orderPlacedHandler
            ...
            .catch((err) => {
            // @ts-expect-error fixme: auto ignored
            return expect(err.errorStack[0].getMsg()).to.be.eq("could not update userDetail with placed order");
        });
    });
