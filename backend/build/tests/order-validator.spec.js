...
test.group("OrderValidator", (group) => {
    ...
    const orderValidator = new OrderValidator(orderItemValidator, orderPlacedValidator, orderFieldValidator, orderUserDetailValidator);
    ...
    test("should reject if amount is null or undefined", async () => {
        ...
        return expect(orderValidator.validate(testOrder, false)).to.eventually.be.rejectedWith(BlError, /order.amount is undefined/);
    });
    test("should reject if branch is not found", async () => {
        testOrder.branch = "notFoundBranch";
        return expect(orderValidator.validate(testOrder, false)).to.be.rejectedWith(BlError, "not found");
    });
    test("should reject if orderItems is empty or undefined", async () => {
        testOrder.orderItems = [];
        return expect(orderValidator.validate(testOrder, false)).to.eventually.be.rejectedWith(BlError, /order.orderItems is empty or undefined/);
    });
    test("should reject with error", async () => {
        orderItemShouldResolve = false;
        return expect(orderValidator.validate(testOrder, false)).to.eventually.be.rejectedWith(BlError, /orderItems not valid/);
    });
    test("should reject with error", async () => {
        orderPlacedShouldResolve = false;
        return expect(orderValidator.validate(testOrder, false)).to.eventually.be.rejectedWith(BlError, /validation of order.placed failed/);
    });
    test("should reject with error", async () => {
        orderUserDetailValidatorShouldResolve = false;
        return expect(orderValidator.validate(testOrder, false)).to.be.rejectedWith(BlError, /validation of UserDetail failed/);
    });
});
