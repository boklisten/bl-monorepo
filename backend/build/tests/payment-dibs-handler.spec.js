...
test.group("PaymentDibsHandler", (group) => {
    ...
    let testPayment;
    let paymentUpdated;
    let getPaymentIdConfirm;
    let testDibsEasyOrder;
    let getDibsEasyOrderConfirm;
    let testPaymentId;
    let orderUpdated;
    let sandbox;
    group.each.setup(() => {
        ...
        sandbox.stub(dibsPaymentService, "orderToDibsEasyOrder").callsFake(() => {
            if (getDibsEasyOrderConfirm)
                return testDibsEasyOrder;
    ...
    test("should reject if order is not found", async () => {
        ...
        return expect(paymentDibsHandler.handleDibsPayment(testPayment)).to.be.rejectedWith(BlError, /order not found/);
    });
    test("should reject if dibsPaymentService.orderToDibsEasyOrder rejects", async () => {
        getDibsEasyOrderConfirm = false;
        return expect(paymentDibsHandler.handleDibsPayment(testPayment)).to.be.rejectedWith(BlError, /could not create dibs easy order/);
    });
