...
test.group("PaymentPatchHook", (group) => {
    ...
    let testPayment;
    let dibsPaymentCreated;
    let paymentValidated;
    let sandbox;
    group.each.setup(() => {
        testPayment = {
            id: "payment1",
    ...
    test("should reject if paymentValidator.validate rejects", async () => {
        ...
        return expect(paymentPatchHook.after([testPayment])).to.be.rejectedWith(BlError, /could not validate payment/);
    });
    ...
    test("should reject if paymentDibsHandler.handleDibsPayment rejects", async () => {
        ...
        return expect(paymentPatchHook.after([testPayment])).to.be.rejectedWith(BlError, /could not create dibs payment/);
    });
    test("should reject with error", async () => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        testPayment.method = "something";
        return expect(paymentPatchHook.after([testPayment])).to.be.rejectedWith(BlError, /payment.method "something" not supported/);
    });
});
