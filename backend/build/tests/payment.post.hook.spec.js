...
test.group("PaymentPostHook", (group) => {
    ...
    test("should reject if ids is empty or undefined", async () => {
        return expect(
        // @ts-expect-error fixme: auto ignored
        paymentPostHook.after([], testAccessToken)).to.eventually.be.rejectedWith(BlError, /payments is empty or undefined/);
    });
    test("should reject if paymentValidator.validate rejects", async () => {
        paymentValidated = false;
        return expect(
        // @ts-expect-error fixme: auto ignored
        paymentPostHook.after([testPayment], testAccessToken)).to.be.rejectedWith(BlError, /payment could not be validated/);
    });
    test("should reject if paymentDibsHandler.handleDibsPayment rejects", async () => {
        ...
        return expect(
        // @ts-expect-error fixme: auto ignored
        paymentPostHook.after([testPayment], testAccessToken)).to.be.rejectedWith(BlError, /could not create dibs payment/);
    });
});
