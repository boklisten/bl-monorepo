...
test.group("RefreshTokenValidator", async () => {
    test("should reject with BlError when refreshToken is empty", async () => {
        const refreshToken = "";
        RefreshTokenValidator.validate(refreshToken).should.be.rejectedWith(BlError);
    });
    ...
    test("should reject with BlCode 909", async () => {
        jwt.sign({ username: "test", iat: Math.floor(Date.now() / 1000) - 10000 }, "test", { expiresIn: "1s" }, (error, refreshToken) => {
            RefreshTokenValidator.validate(refreshToken).catch((rtokenError) => {
    ...
    test("should resolve with payload", async () => {
        ...
        const userid = "abc";
        RefreshTokenCreator.create(username, userid).then((refreshToken) => {
            RefreshTokenValidator.validate(refreshToken).then(
            // @ts-expect-error fixme: auto ignored
            (refreshToken) => {
