import { UserDetailUpdateHook, } from "@backend/lib/collections/user-detail/hooks/user-detail-update.hook.js";
import { test } from "@japa/runner";
import { assert, use as chaiUse, should } from "chai";
import chaiAsPromised from "chai-as-promised";
chaiUse(chaiAsPromised);
should();
const customerAccessToken = { permission: "customer" };
const adminAccessToken = { permission: "admin" };
test.group("UserDetailUpdateHook", async () => {
    ...
    test("should error on wrongly-typed field", async () => {
        ...
        await Promise.all(invalidBodies.map((body) => 
        // Flip promise result, resolved => rejected and vice versa
        new Promise((resolve, reject) => {
            userDetailUpdateHook
                .before(body, adminAccessToken)
                .then(() => reject(new Error(`Validator accepted wrongly typed ${JSON.stringify(body)}`)))
                .catch(() => resolve());
        })));
    });
});
