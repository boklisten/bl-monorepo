...
test.group("UserDetailValidOperation", (group) => {
    ...
    test("should send response with {valid: true}", async ({ assert }) => {
        ...
        const response = await userDetailValidOperation.run(blApiRequest, 
        // @ts-expect-error fixme: auto ignored
        null, null);
    ...
    test("should resolve with valid false if address and postCode is not defined", async ({ assert, }) => {
    ...
    test("should resolve with valid false if postCity and phone is not defined", async ({ assert, }) => {
