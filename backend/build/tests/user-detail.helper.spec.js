...
test.group("UserDetailHelper", (group) => {
    ...
    let testUserDetail;
    let userDetailStorageUpdateSuccess;
    ...
    test(".updateUserDetailBasedOnDibsEasyPayment() should update userDetail with values from dibsEasyPayment", async () => {
        const updatedUserDetail = await userDetailHelper.updateUserDetailBasedOnDibsEasyPayment("userDetail1", testDibsEasyPayment);
        const name = testDibsEasyPayment.consumer.privatePerson.firstName +
            " " +
