...
test.group("UserProvider", (group) => {
    ...
    let userValidStub;
    let createTokenStub;
    let createDefaultLocalLoginStub;
    group.each.setup(() => {
        ...
        createDefaultLocalLoginStub = sandbox.stub(LocalLoginHandler, "createDefaultLocalLoginIfNoneIsFound");
    });
    group.each.teardown(() => {
        sandbox.restore();
    });
    test("loginOrCreate() - should reject if userHandler.valid rejects", async () => {
        ...
        return expect(UserProvider.loginOrCreate("username@mail.com", "local", "abcdef")).to.eventually.be.rejectedWith(BlError, /user is not valid/);
    });
    test("loginOrCreate() - should reject if LocalLoginHandler.createDefaultLocalLoginIfNoneIsFound rejects", async () => {
        ...
        createDefaultLocalLoginStub.rejects(new BlError("local login could not be created"));
        return expect(UserProvider.loginOrCreate("username@mail.com", "local", "abcde")).to.eventually.be.rejectedWith(BlError, /local login could not be created/);
    });
    test("loginOrCreate() - should reject if userHandler.create rejects", async () => {
        ...
        return expect(UserProvider.loginOrCreate("username@mail.com", "local", "abcde")).to.eventually.be.rejectedWith(BlError, /user could not be created/);
    });
    test("loginOrCreate() - should resolve with a user object and tokens", async () => {
        ...
        return expect(UserProvider.loginOrCreate("username@mail.com", "local", "abcdefg")).to.eventually.be.eql({ user: user, tokens: tokens });
    });
    test("loginOrCreate() - should resolve with a user object and tokens", async () => {
