...
import { formatActionsString, UserMatchTitle } from "@/features/matches/matchesList/helper";
...
export default function UserMatchListItem({ userMatch }: { userMatch: UserMatchWithDetails }) {
  ...
  return (
    <MatchListItemCard finished={isCurrentUserFulfilled} matchId={userMatch.id} matchType={"user"}>
      <Title order={4}>
      ...
      <Activity mode={isCurrentUserStarted ? "visible" : "hidden"}>
        <ProgressBar
          ...
          subtitle={
            <Text size={"sm"}>
              {statusText} {currentUserActualItemCount} av {currentUserExpectedItemCount} bøker
            </Text>
          }
        />
      </Activity>
      <Activity mode={!isCurrentUserStarted && !isCurrentUserFulfilled ? "visible" : "hidden"}>
        <Text>{formatActionsString(currentUser.items.length, currentUser.wantedItems.length)}</Text>
      </Activity>
